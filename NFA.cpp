...
/**
 * @brief Función que es llamada desde el constructor para crear el NFA a traves de un fichero de entrada
 * @param in_file 
 */
void NFA::Generate_NFA (string& in_file) {
  ...
  string aux_string;
    ...
    alphabet_.Alphabet_Create(aux_chain);
 ...
 */
void NFA::Write_DFA (DFA dfa, string& file_out) {
  ...
  if (ofs.fail()) {
    ...
    ofs << dfa.get_Acepteded_States().size() << "\n";
    for (State est: dfa.get_Acepteded_States()) {
      ofs << est.get_Name() << "\n";
    }
 ...
 */
void NFA::set_Acepted (const set <State>& acept) {
  acept_ = acept;
}

/**
 * @brief Metodo para obtener el atributo numero de estados
 * @return Devuelve el atributo States_num
 */
