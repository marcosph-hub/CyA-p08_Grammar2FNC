...
/**
 * @brief Establece la siguiente transición
 * @param next Transicion a la que se accedera a continuacion
 */
void Transition::Next_Transition (Transition& next) {
  this -> symbol_ = next.get_Symbol();
  this -> next_state_ = next.get_Next_State();
}
